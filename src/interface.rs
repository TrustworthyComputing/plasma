...
pub fn encode(pt: Point<f64>, code_length: usize) -> String {
    ...
    // Note: avoid padding the code, need to reconstruct later. Remove separator
    // if less or equal than SEPARATOR_POSITION
    if code_length <= SEPARATOR_POSITION {
        rev_code.chars().rev().take(code_length).collect()
    } else {
        rev_code.chars().rev().take(code_length + 1).collect()
    }
}
